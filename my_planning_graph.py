...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if a_node.precond_s_nodes().issubset(self.s_levels[level]):
                a_nodes.add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for s_node in a_node.effect_s_nodes():
                s_nodes.add(s_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return bool(set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)) or \
               bool(set(node_a2.action.effect_rem) & set(node_a1.action.effect_add))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return bool(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) or \
               bool(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)) or \
               bool(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) or \
               bool(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for parent_s1 in node_a1.parents:
            for parent_s2 in node_a2.parents:
                if (parent_s1.is_mutex(parent_s2) or
                        parent_s2.is_mutex(parent_s1)):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_a1 in node_s1.parents:
            for parent_a2 in node_s2.parents:
                if (not parent_a1.is_mutex(parent_a2) or
                        not parent_a2.is_mutex(parent_a1)):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for level, nodes in enumerate(self.s_levels):
            for node in nodes:
                if node.is_pos and node.symbol in goals:
                    level_sum += level
                    goals.remove(node.symbol)
                    if not goals:
                        return level_sum
        return level_sum + len(self.s_levels) * len(goals)
